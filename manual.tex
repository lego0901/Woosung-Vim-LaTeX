...
이루어집니다.} 죄송스럽지만, 리눅스 환경이 아니라면 적당히 메뉴얼을 잘 걸러
해석해주셔야 할 것입니다.. (윈도우 버전은 다음에 시간이 되면 업데이트하겠습니다.)



\section{Vim 설정}
설치는 간단하게 넘어가도록 하겠습니다. Ubuntu 환경에서는 다음과 같이 설치할 수 있습니다.
다음 코드를 터미널에 입력하시면 됩니다. 윈도우나 OS X에서의 환경은 다른 웹페이지에서 따로
참고하시길 바랍니다.
...
LATEX을 쓰는데 편리한 단축키 소스들을 \texttt{.latexrc} 으로 작성하여 첨부하였습니다.
...
\texttt{kotex} 패키지를 각각 추가해줘야합니다. 스타일 파일에 이 패키지를 불러오는 코드를
적고, 이 스타일 파일을 \Verb+\usepackage+ 명령어로 불러옴으로써 함께 로드가 된 것입니다.
이와 같이 자주 사용하게될 긴 명령어들의 목록은 \texttt{sty} 파일로 만들어 미리 저장해두는
것을 권장합니다.
...
가령, \Verb+/etc/texmf/web2c/texmf.cnf+ 로 나왔다고 하면, 이렇게 치시면 됩니다.
\begin{Verbatim}[tabsize=4,xleftmargin=2em]
...
\begin{itemize}
	\item \Verb+\ra+: \Verb+\rightarrow+ \quad ($\ra$)
	\item \Verb+\la+: \Verb+\leftarrow+ \quad ($\la$)
	\item \Verb+\Ra+: \Verb+\rightarrow+ \quad ($\Ra$)
	\item \Verb+\La+: \Verb+\leftarrow+ \quad ($\La$)
	\item \Verb+\R, \N, \Z+: \quad ($\R, \N, \Z$)
	\item \Verb+\ep, \dt+: \quad ($\ep, \dt$)
	\item \Verb+\inv+: \Verb+^{-1}+ \quad ($\inv$)
	\item \Verb+\ie+: \quad (\ie)
	\item \Verb+\eg+: \quad (\eg)
\end{itemize}
...
\begin{itemize}
	...
	\item \textit{[F5] 버튼}: 오류 로그를 오른쪽에 탭으로 띄웁니다.
\end{itemize}
...
\begin{itemize}
	\item \textbf{\bs Package 입력}: \Verb+\newpackage{}+ 텍스트를 입력합니다.
